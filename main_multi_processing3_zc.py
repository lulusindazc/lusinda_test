...
def shangpin_area(shelfid):
    if shelfid == 7:
        ...
        left_x, top_y, right_m, bottom_n = 744, 0, 208, 327
    elif shelfid == 5:
        left_x, top_y, right_m, bottom_n = 677, 39, 380, 286
    elif shelfid == 6:
        left_x, top_y, right_m, bottom_n = 353, 39, 200, 273
    elif shelfid == 3:
        left_x, top_y, right_m, bottom_n = 590, 41, 113, 350
    elif shelfid == 1:
        left_x, top_y, right_m, bottom_n = 513, 138, 273, 387
    else:
...
def initial_flag_out(left, top, right, bottom, shelfid):
    ...
    distance = distanceCal(bottom_n, bottom)
    #print("shelf {}: distance = {}".format(shelfid, distance))
    if calcIOU1 > 0.1 or distance < 130:
        flag = 1
    else:
        flag = 0
    return flag
...
def initial_flag_out_7(left, top, right, bottom, shelfid):
    left_x, top_y, right_m, bottom_n = shangpin_area(shelfid)
    calcIOU1 = newcalcIOU(left, top, right, bottom, left_x, top_y, right_m, bottom_n)
    #print("Shelf {}: Filter out IOU = {}".format(shelfid, calcIOU1))
    distance = distanceCal(bottom_n, bottom)
            ...
            if dic_change.has_key(id_name):
                if dic_change[id_name] == 1:
                    if huojia1_id == 7:
                        ...
                        flag = initial_flag_out_7(left, top, right, bottom, huojia1_id)
                else:
